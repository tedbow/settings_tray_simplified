...
namespace Drupal\Tests\settings_tray_simple\FunctionalJavascript;

use Drupal\block\Entity\Block;
use Drupal\block_content\Entity\BlockContent;
use Drupal\block_content\Entity\BlockContentType;
use Drupal\menu_link_content\Entity\MenuLinkContent;
use Drupal\settings_tray_simple_test\Plugin\Block\SettingsTrayFormAnnotationIsClassBlock;
use Drupal\settings_tray_simple_test\Plugin\Block\SettingsTrayFormAnnotationNoneBlock;
...
class SettingsTrayBlockFormTest extends OffCanvasTestBase {
  ...
  public static $modules = [
    ...
    'toolbar',
    'contextual',
    'settings_tray_simple',
    'quickedit',
    'search',
    'block_content',
    'settings_tray_simple_test',
    // Add test module to override CSS pointer-events properties because they
    // cause test failures.
    'settings_tray_simple_test_css',
    'settings_tray_simple_test',
    'settings_tray_simple_override_test',
    'menu_ui',
    'menu_link_content',
  ];
  ...
  public function testBlocks($theme, $block_plugin, $new_page_text, $element_selector, $label_selector, $button_text, $toolbar_item) {
    ...
    switch ($block_plugin) {
      ...
      case 'settings_tray_simple_test_class':
  ...
  public function providerTestBlocks() {
    ...
    foreach ($this->getTestThemes() as $theme) {
      $blocks += [
        ...
        // \Drupal\Tests\settings_tray_simple\Functional\SettingsTrayTest::testPossibleAnnotations().
        "$theme: " . SettingsTrayFormAnnotationIsClassBlock::class => [
          'theme' => $theme,
          'block_plugin' => 'settings_tray_simple_test_class',
          'new_page_text' => NULL,
        ...
        "$theme: " . SettingsTrayFormAnnotationNoneBlock::class => [
          'theme' => $theme,
          'block_plugin' => 'settings_tray_simple_test_none',
          'new_page_text' => NULL,
          'element_selector' => 'span',
          'label_selector' => NULL,
          'button_text' => NULL,
   ...
   * "Quick edit settings" is settings_tray_simple.module link.
   */
  ...
  public function testValidationMessages() {
    ...
    foreach ($this->getTestThemes() as $theme) {
      $this->enableTheme($theme);
      $block = $this->placeBlock('settings_tray_simple_test_validation');
      $this->drupalGet('user');
      $this->enableEditMode();
      $this->openBlockForm($this->getBlockSelector($block));
      $page->pressButton('Save Block with validation error');
      $web_assert->assertWaitOnAjaxRequest();
      // The settings_tray_simple_test_validation test plugin form always has a
      // validation error.
  ...
  public function testOverriddenBlock() {
    ...
    // overridden.
    $this->container->get('state')->set('settings_tray_simple_override_test.block', TRUE);
  ...
  public function testOverriddenConfigurationRemoved() {
    ...
    $this->container->get('state')->set('settings_tray_simple_override_test.site_name', TRUE);
    $this->drupalGet('user');
    $this->openBlockForm($this->getBlockSelector($branding_block));
    ...
    // overridden.
    $this->container->get('state')->set('settings_tray_simple_override_test.menu', TRUE);
    $this->drupalGet('user');
    $web_assert->pageTextContains('This is on the menu');
