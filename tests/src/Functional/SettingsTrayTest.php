...
namespace Drupal\Tests\settings_tray_simple\Functional;
...
class SettingsTrayTest extends BrowserTestBase {
  ...
  /**
   * Tests the 3 possible forms[settings_tray_simple] annotations: class, FALSE, none.
   *
  ...
  public function testPossibleAnnotations() {
    $test_block_plugin_ids = [
      // Block that explicitly provides an "settings_tray_simple" form class.
      'settings_tray_simple_test_class',
      // Block that explicitly provides no "settings_tray_simple" form, thus opting out.
      'settings_tray_simple_test_false',
      // Block that does nothing explicit for Settings Tray.
      'settings_tray_simple_test_none',
    ];
    ...
    foreach ($placed_blocks as $plugin_id => $placed_block) {
      ...
      // All blocks except 'settings_tray_simple_test_false' are editable. For more
      // detailed test coverage, which requires JS execution, see
      // \Drupal\Tests\settings_tray_simple\FunctionalJavascript\SettingsTrayBlockFormTest::testBlocks().
      if ($plugin_id === 'settings_tray_simple_test_false') {
  ...
  public function testOptOut() {
    ...
    $excluded_block_plugin_ids = ['page_title_block', 'system_main_block', 'settings_tray_simple_test_false'];
    $block_selectors = [];
    ...
    // Assert that each block that has a "forms[settings_tray_simple] = FALSE" annotation:
    // - is still rendered on the page
    // - but is not marked as "editable" by settings_tray_simple_preprocess_block()
